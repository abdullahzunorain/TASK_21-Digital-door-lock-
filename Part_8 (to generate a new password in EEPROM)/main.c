   ...
   while(1)                                    
    {
        ...
        {
            case Generate_Password:
                LCD_Clear();  // Clear the LCD before showing the new message
                LCD_ShowString(1, 0, "Generate Password");  // Display "Generate Password"
                if (j < 6)
                {
                    RandomPassword();
                    j++;
                ...
                {  
                    EEPROM_Write(207, 0xAA);  // Store the generated password
                    current_state = Enter_Password;  // Change state to Enter_Password
                    Timmer_Delay(3000);  // Delay before switching state
                }
                break;
//
